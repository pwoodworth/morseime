...
//        }
  ...
  public void getWords(final WordComposer codes, final WordCallback callback) {
    ...
    if (ENABLE_MISSED_CHARACTERS && count < 5) {
      for (int skip = 0; skip < codesSize; skip++) {
        int tempCount = getSuggestionsNative(mNativeDict, mInputCodes, codesSize, mOutputChars, mFrequencies,
  ...
  private int openNative(AssetManager am, String resourcePath, int typedLetterMultiplier, int fullWordMultiplier) {
    return 0;
  }

  private void closeNative(int dict) {

  }

  private boolean isValidWordNative(int nativeData, char[] word, int wordLength) {
    return false;
  }

  private int getSuggestionsNative(int dict,
                                   int[] inputCodes,
                                   int codesSize,
                                   char[] outputChars,
                                   int[] frequencies,
